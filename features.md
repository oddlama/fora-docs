...
### ...focuses a remote scripting API and not an entire environment.

Keeping the tool lightweight increases maintainability for everyone involved.
Fewer things can change or break, and fewer things to keep updated.
...
you sometimes you want to replace it with another dictionary, and other times just
want to add to it. As group evaluation order is well-defined in Fora, it can
just give you full access to the variable definitions. As all variables are just
...
# group have no defined ordering, concurrent modification is allowed as
# it doesn't create ambiguity as long as the modification doesn't remove
# or modify existing information (this is also hard to detect).
# Altough if the desktop group would overwrite `system_users`, Fora
# would detect this as an error.
system_users["www"] = {"home": "/var/www", "system": True}
```
{% endtab %}
{% tab title="hosts/minimal_proxy.py" %}
```python
# The proxy server is special and should not modify any users.
...
Therefore, I want the option to change the defaults for some of those regularly used parameters.
...
Did you ever find yourself in need to shell-escape a command? Me neither.
I want the equivalent of a remote `subprocess.run`, for both internal use and use in scripts.
That way I don't have to worry about escaping command arguments, but still have the option
to spawn a shell with `["bash", "-c", "<shellcommand>"]` when I need to.
...
For example a `deploy_website.py` script that automatically creates nginx configuration but required
a path to the webroot that should be deployed. Fora allows you to pass regular python objects as script parameters
by adding a simple global namespace:
...
which can be reused for all commands.
...
By default, Fora displays only each executed operation and each state parameter that changed.
